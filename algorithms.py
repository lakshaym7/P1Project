...
def initialize_arima_models(ip_set):
    arima_models = []
    for ip_data in ip_set:
        # Check for missing values and handle them
        ip_data = np.array(ip_data)
        if np.any(np.isnan(ip_data)):
            print("Warning: NaN values detected in data, filling with previous values")
            ip_data = pd.Series(ip_data).fillna(method='ffill').values  # Forward fill for missing values
        
        # Ensure data is stationary (first difference if needed)
        if not is_stationary(ip_data):
            ip_data = np.diff(ip_data)  # Differencing to make data stationary
        
        # Fit ARIMA model
        try:
            # Choose appropriate (p, d, q) for your data; you may adjust these values
            model = ARIMA(ip_data, order=(1, 1, 1))
...
def is_stationary(data):
    ...
    return adf_test[1] <= 0.05  # p-value <= 0.05 indicates stationarity
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                # Matching keypoints for single camera input
                ...
                # Safely accessing activity_dict with prediction, adding default message if out of bounds
                activity_label = activity_dict.get(int(prediction) + 5, "Unknown Activity")
                dict_frames[0]["tagged_df"]["text"] += f" Pred: {activity_label}"
                ...
                # Processing for dual camera input with multiple matches and predictions
                ...
                # Handling unmatched indices for two camera streams
                ...
                # Removing wrongly matched elements and updating matched lists
                unmatch_previous = remove_wrongly_matched(ip_sets[0][:num_matched], ip_sets[1][:num_matched])
                if unmatch_previous:
                ...
                # Adding matched pairs back to ip_sets and lstm_sets
                for i in sorted(new_pairs[0], reverse=True):
                    ip_sets[0].pop(i)
                    lstm_sets[0].pop(i)
                for i in sorted(new_pairs[1], reverse=True):
                    ip_sets[1].pop(i)
                    lstm_sets[1].pop(i)
                ...
                num_matched += len(new_matched1)

                # Generating predictions and displaying them on both frames
                ...
                activity_label1 = activity_dict.get(prediction1 + 5, "Unknown Activity")
                activity_label2 = activity_dict.get(prediction2 + 5, "Unknown Activity")
                dict_frames[0]["tagged_df"]["text"] += f" Pred: {activity_label1}"
                dict_frames[1]["tagged_df"]["text"] += f" Pred: {activity_label2}"
                ...
                # Assertions to ensure alignment between ip_sets and lstm_sets
...
def get_all_features(ip_set, arima_set, model):
    valid_idxs = []
    invalid_idxs = []
    ...
    for i, ips in enumerate(ip_set):
        ...
        if ips[-1] is None:
            ...
            ips[-1]["features"] = {}
            # Get features for height_bbox, ratio_bbox, angle, etc.
        ...
        # Create feature vector (for ARIMA)
        xdata = []
        if ips[-1] is None:
            if last1 is None:
        ...
        arima_model = arima_set[i]
        xdata = np.array(xdata)
        
        # Train ARIMA model on past features and forecast next value
        # Fit ARIMA model to historical data (assuming previous data is available)
        if len(xdata) > 1:  # Ensure we have more than one data point for ARIMA
            try:
                arima_model = ARIMA(xdata, order=(p, d, q))  # Adjust p, d, q as per your requirements
                arima_model = arima_model.fit()
                forecast = arima_model.forecast(steps=1)  # Forecast the next value
                prediction = forecast[0]
            except Exception as e:
                print(f"ARIMA fitting failed for index {i}, error: {e}")
                prediction = 15  # If ARIMA fitting fails, assign a default value
        else:
            prediction = 15  # If not enough data, assign a default value
